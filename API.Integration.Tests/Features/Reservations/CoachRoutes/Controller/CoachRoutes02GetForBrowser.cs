...
namespace CoachRoutes {
    ...
    [Collection("Sequence")]
    public class CoachRoutes02GetForBrowser : IClassFixture<AppSettingsFixture> {
        ...
        private readonly string _url = "/coachRoutes/getForBrowser";

        #endregion

        public CoachRoutes02GetForBrowser(AppSettingsFixture appsettings) {
        ...
        public async Task Active_Users_Can_Get_Active(Login login) {
            ...
            var records = JsonSerializer.Deserialize<List<CoachRouteBrowserVM>>(await actionResponse.Content.ReadAsStringAsync(), new JsonSerializerOptions { PropertyNameCaseInsensitive = true });
