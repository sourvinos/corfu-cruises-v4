...
namespace PickupPoints {
    ...
    [Collection("Sequence")]
    public class PickupPoints02GetForBrowser : IClassFixture<AppSettingsFixture> {
        ...
        private readonly string _url = "/pickupPoints/getForBrowser";

        #endregion

        public PickupPoints02GetForBrowser(AppSettingsFixture appsettings) {
        ...
        public async Task Active_Users_Can_Get_Active(Login login) {
            ...
            var records = JsonSerializer.Deserialize<List<PickupPointBrowserVM>>(await actionResponse.Content.ReadAsStringAsync(), new JsonSerializerOptions { PropertyNameCaseInsensitive = true });
