...
namespace ShipOwners {
    ...
    [Collection("Sequence")]
    public class ShipOwners02GetForBrowser : IClassFixture<AppSettingsFixture> {
        ...
        private readonly string _url = "/shipOwners/getForBrowser";

        #endregion

        public ShipOwners02GetForBrowser(AppSettingsFixture appsettings) {
        ...
        public async Task Active_Users_Can_Get_Active(Login login) {
            ...
            var records = JsonSerializer.Deserialize<List<ShipOwnerBrowserVM>>(await actionResponse.Content.ReadAsStringAsync(), new JsonSerializerOptions { PropertyNameCaseInsensitive = true });
