...
namespace Drivers {
    ...
    [Collection("Sequence")]
    public class Drivers02GetForBrowser : IClassFixture<AppSettingsFixture> {
        ...
        private readonly string _url = "/drivers/getForBrowser";

        #endregion

        public Drivers02GetForBrowser(AppSettingsFixture appsettings) {
        ...
        public async Task Active_Users_Can_Get_Active(Login login) {
            ...
            var records = JsonSerializer.Deserialize<List<DriverBrowserVM>>(await actionResponse.Content.ReadAsStringAsync(), new JsonSerializerOptions { PropertyNameCaseInsensitive = true });
