...
namespace Ports {
    ...
    [Collection("Sequence")]
    public class Ports02GetForBrowser : IClassFixture<AppSettingsFixture> {
        ...
        private readonly string _url = "/ports/getForBrowser";

        #endregion

        public Ports02GetForBrowser(AppSettingsFixture appsettings) {
        ...
        public async Task Active_Users_Can_Get_Active(Login login) {
            ...
            var records = JsonSerializer.Deserialize<List<PortBrowserVM>>(await actionResponse.Content.ReadAsStringAsync(), new JsonSerializerOptions { PropertyNameCaseInsensitive = true });
